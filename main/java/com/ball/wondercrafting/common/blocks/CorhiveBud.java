...
public class CorhiveBud extends Corgrass implements IGrowable {

    public static final IntegerProperty STAGE = BlockStateProperties.STAGE_0_1;
    protected static final VoxelShape SHAPE = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 12.0D, 14.0D);
    private final Tree tree;

    public CorhiveBud(Tree treeIn, AbstractBlock.Properties properties) {
        super(properties);
        this.tree = treeIn;
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        if (worldIn.getLight(pos.up()) >= 1 && random.nextInt(7) == 0) {
    ...
    public void placeTree(ServerWorld world, BlockPos pos, BlockState state, Random rand) {
        if (state.get(STAGE) == 0) {
            ...
            world.setBlockState(pos.down(), BlockInit.CORDIRT.get().getDefaultState());
        }
